...
const PANELS = {
  "PBR Panel": {
    gauges: ["22", "24", "26", "29"],
    desc: '36" coverage, 12" OC ribs, 1¼" rib height',
    trimCategory: "PBR",
  },
  "PBU Panel": {
    gauges: ["24", "26"],
    desc: '36" coverage, through-fastened',
    trimCategory: "PBR",
  },
  MaxPanel: {
    gauges: ["26", "29"],
    desc: '36" coverage, 9" ribs, ¾" rib height',
    trimCategory: "Max Panel",
  },
  "MaxStrong Panel": {
    gauges: ["26", "29"],
    desc: "Through-fastened, classic look",
    trimCategory: "Max Panel",
  },
  "MaxLoc100 Panel": {
    gauges: ["24", "26"],
    desc: '16" coverage, 7/8" rib, concealed fastener',
    trimCategory: "Standing Seam",
  },
  "MaxSeam175 Panel": {
    gauges: ["22", "24"],
    desc: '14" coverage, 1¾" rib, hidden clip',
    trimCategory: "Standing Seam",
  },
  "MaxSnap150 Panel": {
    gauges: ["22", "24"],
    desc: "Hidden clip, snap-lock standing seam",
    trimCategory: "Standing Seam",
  },
  "MaxMech150 Panel": {
    gauges: ["22", "24"],
    desc: "Mechanical seam, concealed fasteners",
    trimCategory: "Standing Seam",
  },
  "MaxMech200 Panel": {
    gauges: ["22", "24"],
    desc: '2" mechanical seam, low-slope capable',
    trimCategory: "Standing Seam",
  },
  "Board & Batten Panel": {
    gauges: ["26", "29"],
    desc: '16" coverage, concealed fastener siding',
    trimCategory: "Max Panel",
  },
  "5V Crimp": {
    gauges: ["26", "29"],
    desc: "Traditional style, residential/agricultural",
    trimCategory: "Max Panel",
  },
};
...
const COLORS_BY_GAUGE = {
  "22": [
    "Acrylic Coated Galvalume","Charcoal Gray","Dark Bronze","Dove Gray",
    "Mansard Brown","Matte Black","Medium Bronze","Regal White","Slate Gray",
  ],
  "24": [
    "Acrylic Coated Galvalume","Ash Gray","Burgundy","Burnished Slate",
    "Champagne","Charcoal Gray","Colonial Red","Copper","Cor-ten AZP Raw",
    "Dark Bronze","Dove Gray","Evergreen","Hartford Green","Hemlock Green",
    "Mansard Brown","Matte Black","Medium Bronze","Patina Green",
    "Pre-Weathered Galvalume","Regal Blue","Regal Red","Regal White",
    "Sandstone","Sierra Tan","Silver","Slate Blue","Slate Gray","Solar White",
    "Stone White","Surrey Beige","Terra Cotta","TLG Black","TLG Charcoal Gray",
    "TLG Dark Bronze","TLG Medium Bronze","TLG Moonstone","Vintage",
  ],
  "26": [
    "Alamo White","Ash Gray","Black","Brite Red","Brown","Brilliant White",
    "Burgundy","Burnished Slate","Charcoal","Copper Penny","Fern Green",
    "Gallery Blue","Galvalume","Hunter Green","Light Stone","Ocean Blue",
    "Pewter Gray","Polar White","Rustic Red","Tan","Taupe",
    "Regal White","Dove Gray","Slate Gray","Dark Gray","Charcoal Gray",
    "Evergreen","Antique","Colonial Red","Terra Cotta","Dark Bronze",
    "Medium Bronze","Sandstone","Sierra Tan","Mansard Brown",
...
const TRIM_BY_CATEGORY = {
  "Standing Seam": [
    "14Z Bar","16Z Bar","Box Rake","Cleat","Counter Flashing","End Wall",
    "Flush Eave","Hip Cap","Offset Cleat","Pitch Change","Plumb Eave",
    "Reglet Flashing","Side Wall","Single Slope Ridge","Square Eave",
    "Step Rake 14","Step Rake 16","Step Ridge","Valley","Vent Retainer",
  ],
  PBR: [
    "Base Trim","Box Rake","Double Angle","End Wall","Flat Sheet",
    "Formed Ridge Cap","Hi-Side Eave","Hi-Side Parapet","House Rake",
    "Inside Corner","Inside Single Angle","J-Trim","Jamb Header","Jamb Trim",
    "Long Eave Trim","Outside Corner","Outside Single Angle","Rake","Rat Guard",
    "Short Eave","Side Wall","Skylight Trim","Tie-In","Universal Ridge",
    "Valley","Wide Valley","Window Cap",
  ],
  "Max Panel": [
    "Barn Rake","Barn Ridge","Door Edge","Door Jamb Wide","Door Post",
    "Double Angle","End Wall","Fascia","Flat Sheet","Gable","Gutter Apron",
    "Inside Corner","Inside Single Angle","J-Trim","Keystone","Large Corner",
    "Lower Gambrel","Outside Single Angle","Overhead Door Jamb",
    "OH Door Jamb w/ Drip Edge","Rat Guard","Residential Drip Edge",
    "Residential Eave","Residential Hip Cap","Residential Rake",
    "Residential Ridge Cap","Residential Valley","Round Track Cover",
    "Side Wall","Small Corner","Soffit","Square Base",
    "Square Track Cover Narrow","Upper Gambrel","Wide Ridgecap","Window Cap",
  ],
};
...
const newPanel = (defaults = {}) => ({ id: Date.now() + Math.random(), panel: "", gauge: "", color: "", pieceCount: 0, feet: "", inches: "", ...defaults });
...
function NumberPad({ value, onChange, label }) {
  ...
  return (
    <div>
      ...
      <div style={styles.numDisplay} onClick={() => setShow(true)}>
        {value || 0}
      </div>
      {show && (
            ...
            <div style={styles.numPadGrid}>
              {["1","2","3","4","5","6","7","8","9","DEL","0","OK"].map((k) => (
                <button key={k} onClick={() => handleKey(k)}
                  style={{ ...styles.numPadBtn, ...(k === "OK" ? styles.numPadOk : {}), ...(k === "DEL" ? styles.numPadDel : {}) }}>
                  {k}
                </button>
              ))}
            </div>
...
function Picker({ label, options, value, onChange, placeholder }) {
  ...
  return (
    <div>
      <label style={styles.label}>{label}</label>
      <div style={{ ...styles.pickerBtn, ...(value ? {} : { color: "#999" }) }}
            ...
            <div style={styles.pickerScroll}>
              {options.map((o) => (
                <div key={o} onClick={() => { onChange(String(o)); setShow(false); }}
                  style={{ ...styles.pickerItem, ...(String(o) === String(value) ? styles.pickerItemActive : {}) }}>
                  {o}
                </div>
              ))}
            </div>
...
function PanelEntryCard({ entry, index, total, onUpdate, onRemove }) {
  ...
  const update = (field, value) => {
    ...
    if (field === "panel") { updated.gauge = ""; updated.color = ""; }
    if (field === "gauge") { updated.color = ""; }
    onUpdate(updated);
  };

  const isComplete = entry.panel && entry.gauge && entry.color && entry.pieceCount > 0 && entry.feet !== "";
  ...
  return (
    <div style={styles.panelCard}>
      <div style={styles.panelCardHeader}>
        ...
        {total > 1 && (
          <button style={styles.panelCardRemove} onClick={() => onRemove(entry.id)}>✕ Remove</button>
        )}
      </div>
      ...
      {entry.panel && (
        <Picker label="Gauge *" options={availableGauges} value={entry.gauge} onChange={(v) => update("gauge", v)} placeholder="Select gauge..." />
      )}
      {entry.gauge && (
        <Picker label="Color *" options={availableColors} value={entry.color} onChange={(v) => update("color", v)} placeholder="Select color..." />
      )}
      {entry.color && (
        <>
          ...
          <div style={styles.row}>
            <div style={{ flex: 1 }}>
              <Picker label="Feet *" options={FEET_OPTIONS} value={entry.feet} onChange={(v) => update("feet", v)} placeholder="Ft" />
            </div>
            <div style={{ flex: 1 }}>
              <Picker label="Inches" options={INCHES_OPTIONS} value={entry.inches} onChange={(v) => update("inches", v)} placeholder="In" />
            </div>
          </div>
        </>
      )}
...
function MetalMaxOrderForm() {
  ...
  // Section 2: Panel Selection — array of panels
  ...
  // Section 5: Custom Trim + Comments
  ...
  const availableTrim = allTrimCategories.reduce((acc, cat) => {
    (TRIM_BY_CATEGORY[cat] || []).forEach((t) => { if (!acc.includes(t)) acc.push(t); });
    return acc;
  }, []);
  const trimGauge = firstComplete?.gauge;
  const trimColorOptions = trimGauge ? (COLORS_BY_GAUGE[trimGauge] || []).sort() : [];

  const updatePanelEntry = (updated) => {
    setPanelEntries((prev) => prev.map((p) => (p.id === updated.id ? updated : p)));
  };
  const removePanelEntry = (id) => {
    setPanelEntries((prev) => prev.filter((p) => p.id !== id));
  };
  const addPanelEntry = () => {
    const last = panelEntries[panelEntries.length - 1];
    setPanelEntries((prev) => [...prev, newPanel({
      panel: last?.panel || "",
      gauge: last?.gauge || "",
      color: last?.color || "",
    })]);
  ...
  const allPanelsValid = panelEntries.length > 0 && panelEntries.every(
    (p) => p.panel && p.gauge && p.color && p.pieceCount > 0 && p.feet !== ""
  );

  const toggleTrim = (t) => {
    setTrimItems((prev) => {
      const existing = prev.find((i) => i.name === t);
      if (existing) return prev.filter((i) => i.name !== t);
      return [...prev, { name: t, qty: 1, feet: "", inches: "" }];
    });
  };

  const updateTrimItem = (name, field, value) => {
    setTrimItems((prev) =>
  ...
  const STEPS = [
    { title: "Customer Info", valid: customerName.trim() },
  ...
  const generatePDF = async () => {
    ...
    const trimColorDisplay = sameColorTrim ? `Same as panel (${firstPanel?.color || "N/A"})` : trimColor;
    ...
    const accessories = [];
    if (closureStrips) accessories.push("Closure Strips");
    if (screws) accessories.push("Screws");
    if (solarSeal) accessories.push("Solar Seal");
    if (butylTape) accessories.push("Butyl Tape");

    let sketchDataUrl = "";
    if (hasCustomTrim && canvasRef.current) {
      sketchDataUrl = canvasRef.current.toDataURL("image/png");
    }

    const panelsHTML = panelEntries.map((p, i) => `
      <tr>
        <td style="padding:6px 10px;border-bottom:1px solid #ddd">${i + 1}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #ddd">${p.panel}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #ddd;text-align:center">${p.gauge}ga</td>
        <td style="padding:6px 10px;border-bottom:1px solid #ddd">${p.color}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #ddd;text-align:center">${p.pieceCount}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #ddd;text-align:center">${p.feet}'${p.inches ? ` ${p.inches}"` : ""}</td>
      </tr>
    `).join("");

    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>MetalMax Order - ${customerName}</title>
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      body { font-family: 'Helvetica Neue', Arial, sans-serif; padding: 30px; color: #1a1a2e; font-size: 13px; }
      .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 3px solid #c0392b; padding-bottom: 15px; margin-bottom: 20px; }
      .header h1 { font-size: 22px; color: #c0392b; letter-spacing: 1px; }
      .header .date { font-size: 12px; color: #666; }
      .section { margin-bottom: 18px; }
      .section-title { font-size: 14px; font-weight: 700; color: #c0392b; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; padding-bottom: 4px; border-bottom: 1px solid #eee; }
      .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 20px; }
      .info-row { display: flex; gap: 6px; }
      .info-label { font-weight: 600; min-width: 100px; }
      table { width: 100%; border-collapse: collapse; }
      th { background: #c0392b; color: white; padding: 8px 10px; text-align: left; font-size: 12px; }
      .acc-list { display: flex; gap: 15px; flex-wrap: wrap; }
      .acc-item { padding: 5px 12px; background: #f8f8f8; border: 1px solid #ddd; border-radius: 4px; font-size: 12px; }
      .sketch-img { max-width: 100%; border: 1px solid #ccc; margin-top: 8px; }
      .comments-box { background: #f8f8f8; border: 1px solid #ddd; border-radius: 4px; padding: 10px; white-space: pre-wrap; font-size: 12px; min-height: 40px; }
      @media print { body { padding: 15px; } }
    </style></head><body>
      <div class="header">
        <h1>METALMAX ORDER FORM</h1>
      ...
      <div class="section">
        <div class="section-title">Panel Order${panelEntries.length > 1 ? ` (${panelEntries.length} panels)` : ""}</div>
        <table>
          <thead><tr><th>#</th><th>Panel</th><th style="text-align:center">Gauge</th><th>Color</th><th style="text-align:center">Qty</th><th style="text-align:center">Length</th></tr></thead>
          <tbody>${panelsHTML}</tbody>
        </table>
      </div>
      <div class="section">
        <div class="section-title">Trim Pieces</div>
        <div class="info-row" style="margin-bottom:8px"><span class="info-label">Trim Color:</span> ${trimColorDisplay || "N/A"}</div>
        <table>
          <thead><tr><th>Trim Piece</th><th style="text-align:center">Qty</th><th style="text-align:center">Length</th></tr></thead>
          <tbody>${trimListHTML}</tbody>
        </table>
      </div>
      <div class="section">
        <div class="section-title">Accessories</div>
        ${accessories.length > 0 ? `<div class="acc-list">${accessories.map((a) => `<div class="acc-item">✓ ${a}</div>`).join("")}</div>` : '<p style="color:#999">None selected</p>'}
      </div>
    ...
    <div style={styles.wrapper}>
      <div style={styles.topBar}>
        <div style={styles.logo}>
          <span style={styles.logoM}>METAL</span><span style={styles.logoMax}>MAX</span>
        </div>
        <div style={styles.topBarSub}>Order Entry</div>
      </div>
      ...
      <div style={styles.content}>
        ...
        {step === 0 && (
          <div style={styles.section}>
            <SectionHeader number="1" title="Customer Information" subtitle="Name and PO number" />
            <div style={styles.fieldGroup}>
              <label style={styles.label}>Customer Name *</label>
              <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)}
                style={styles.input} placeholder="Enter customer name" />
            </div>
            <div style={styles.fieldGroup}>
              <label style={styles.label}>PO Number</label>
              <input type="text" value={poNumber} onChange={(e) => setPoNumber(e.target.value)}
                style={styles.input} placeholder="Enter PO number" />
            </div>
          </div>
        )}

        {/* ─── STEP 1: PANEL SELECTION (multi-panel) ─── */}
        {step === 1 && (
          <div style={styles.section}>
            ...
            {panelEntries.map((entry, idx) => (
              <PanelEntryCard
                ...
                entry={entry}
                index={idx}
                total={panelEntries.length}
                onUpdate={updatePanelEntry}
                onRemove={removePanelEntry}
              />
            ))}
          ...
          <div style={styles.section}>
            ...
            {availableTrim.length === 0 ? (
              ...
              <>
                ...
                {!sameColorTrim && (
                  <Picker label="Trim Color" options={trimColorOptions} value={trimColor} onChange={setTrimColor} placeholder="Select trim color..." />
                )}
                ...
                <div style={styles.trimGrid}>
                  {availableTrim.map((t) => {
                    ...
                    return (
                      <div key={t}>
                        <div style={{ ...styles.trimChip, ...(selected ? styles.trimChipActive : {}) }}
                          onClick={() => toggleTrim(t)}>
                        ...
                        {selected && (
                          <div style={styles.trimDetail}>
                            <div style={{ flex: "0 0 60px" }}>
                              <NumberPad label="Qty" value={selected.qty} onChange={(v) => updateTrimItem(t, "qty", v)} />
                            </div>
                            <div style={{ flex: 1 }}>
                              <Picker label="Ft" options={FEET_OPTIONS} value={selected.feet} onChange={(v) => updateTrimItem(t, "feet", v)} placeholder="Ft" />
                            </div>
                            <div style={{ flex: 1 }}>
                              <Picker label="In" options={INCHES_OPTIONS} value={selected.inches} onChange={(v) => updateTrimItem(t, "inches", v)} placeholder="In" />
                            </div>
                          </div>
                        )}
                      </div>
          ...
          <div style={styles.section}>
            ...
            <div style={styles.fieldGroup}>
              ...
              <textarea
                ...
                placeholder="Enter any extra measurements, special instructions, notes for the office..."
                rows={5}
              />
            </div>
            ...
            <div style={styles.summaryBox}>
              ...
              {panelEntries.map((p, i) => (
                <div key={p.id}>
                  <div style={styles.summaryRow}>
                    <span style={{ fontWeight: 600 }}>Panel {i + 1}:</span>
              ...
              <div style={styles.summaryRow}><span>Accessories:</span><strong>
                {[closureStrips && "Closure", screws && "Screws", solarSeal && "Solar Seal", butylTape && "Butyl"].filter(Boolean).join(", ") || "None"}
              </strong></div>
              {comments.trim() && (
      ...
      <div style={styles.navBar}>
        {step > 0 && (
          <button style={styles.navBack} onClick={() => setStep(step - 1)}>← Back</button>
        )}
        <div style={{ flex: 1 }} />
        {step < 4 ? (
...
const styles = {
  ...
  trimChip: { padding: "10px 12px", background: "#fff", border: "2px solid #e0e0dc", borderRadius: 8, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 },
